...
public class Site extends AppCompatActivity {
    ...
    public void on_Modify_PB_Click(View view) {
        Intent Table_Modify = new Intent(this, Site_Modify_HSM.class);
        startActivity(Table_Modify);
    }

    public void on_DownLoad_PB_Click(View view) {
        DownLoad_Complete_Table_from_Cloud_Act_Site = true;
    }

    public void on_Delete_Data_PB_Click(View view) {
        final String Table = "Site";
        ...
        new Thread() {
            public void run() {
                Table_Display_Handler.post(new Runnable() {
                    ...
                    public void run() {
                        String Table = "Site";
                        Cursor OutputC =  myDB_G.ReadData_from_DB_Table(Table, "_id, Type,Owner, Name, Material, Distance,  " +
                                "Rate_Local, Rate_Remote, CFT_Rate, GST_Included, Disabled, Extra, Latitude, Longitude, Create_TS, Modify_TS, User", "", "", "Name");

                        if (OutputC != null) {
                            String[] fromFieldNames1 = {"_id", "Type","Owner", "Name", "Material", "Distance", "Rate_Local", "Rate_Remote", "CFT_Rate",
                                    "GST_Included", "Disabled", "Extra",  "Latitude", "Longitude", "Create_TS", "Modify_TS", "User"};
                            ...
                            int[] ToViewIDs = new int[]{R.id.site_L_id, R.id.site_Type, R.id.site_Owner, R.id.site_Name, R.id.site_Material,
                                    R.id.site_Distance, R.id.site_Rate_Local,  R.id.site_Rate_Remote,  R.id.site_CFT_Rate,  R.id.site_GST_Included,
                                    R.id.site_Disabled,  R.id.site_Extra,
                                    R.id.site_Latitude,  R.id.site_Longitude , R.id.site_Create_TS,  R.id.site_Modify_TS, R.id.site_User};

                            SimpleAdapter k1 = new SimpleAdapter(getBaseContext(), val1, R.layout.site_inner, fromFieldNames1, ToViewIDs);

                            ListView mySite_Items1 = (ListView) findViewById(R.id.Material_Header);
                            mySite_Items1.setAdapter(k1);

                            SimpleCursorAdapter myCursorAdapter;
                            myCursorAdapter = new SimpleCursorAdapter(getBaseContext(), R.layout.site_inner, OutputC, fromFieldNames1, ToViewIDs, 0);
