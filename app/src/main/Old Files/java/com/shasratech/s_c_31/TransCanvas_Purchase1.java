...
import static com.shasratech.s_c_31.MainActivity.Cash_Type_View_set_Text;

public class TransCanvas_Purchase1 extends View {
    ...
    public TransCanvas_Purchase1(Context context) {
        super(context);
    }

    public TransCanvas_Purchase1(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public TransCanvas_Purchase1(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public TransCanvas_Purchase1(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void onDraw(Canvas cv) {
        ...
        if (Purchase1_Chart_Type_Changed || DB_Purchase1_Chart_Data_Changed) {
            switch (main_chart_selection) {
                case MCS_Daily:
                    ...
                    //Statement = "select ROUND(SUM(Total_Val)), Cust_Suplr   from Transactions where In_Out = 'PURCHASE' "
            ...
            Statement = "select ROUND(SUM(Met_Weight)), Cust_Suplr from Transactions where In_Out = 'PURCHASE' "
                    + " and Create_TS >= '" + From_TS + "' and Create_TS <= '" + To_TS + "'  group by 2 order by 2;";

            if (myDB_G.RunQueryOnce(Statement, Result)) {
                Purchase1_Chart_Type_Changed = false;
                DB_Purchase1_Chart_Data_Changed = false;
                ...
                for (; a < b; a++) {
                    ...
                    if ((O2.length > 1) && (O2[1] != null)) {
                        Material = O2[1];
                        int max_len = Material.length();
                        max_len = (max_len >= 10)?10:max_len;
                        Material = Material.substring(0, max_len);
                    }
