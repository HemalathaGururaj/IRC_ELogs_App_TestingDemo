...
public class TransCanvas_Inven_Purchase1 extends View {
    ...
    public TransCanvas_Inven_Purchase1(Context context) {
        super(context);
    }

    public TransCanvas_Inven_Purchase1(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public TransCanvas_Inven_Purchase1(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public TransCanvas_Inven_Purchase1(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void onDraw(Canvas cv) {
        ...
        if (Inven_Purchase_Chart_Type_Changed || DB_Inven_Purchase_Chart_Data_Changed) {
            ...
            Statement = "select ROUND(SUM(Rate * Init_Quantity)), Supplier from Inventory where In_Out = 'IN' "
                    + " and Create_Time >= '" + From_TS + "' and Create_Time <= '" + To_TS + "'  group by 2 order by 2;";

            if (myDB_G.RunQueryOnce(Statement, Result)) {
                Inven_Purchase_Chart_Type_Changed = false;
                DB_Inven_Purchase_Chart_Data_Changed = false;
