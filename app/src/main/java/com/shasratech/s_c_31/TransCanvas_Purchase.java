...
public class TransCanvas_Purchase extends View {
    private final String TAG = "TransCanvasPurchase";
    ...
    public TransCanvas_Purchase(Context context) {
        super(context);
    }

    public TransCanvas_Purchase(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public TransCanvas_Purchase(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public TransCanvas_Purchase(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void onDraw(Canvas cv) {
        ...
        if (Purchase_Chart_Type_Changed || DB_Purchase_Chart_Data_Changed) {
            switch (main_chart_selection) {
                case MCS_Daily:
                    Purchase_Type_View_set_Text("\u20B9 " + "Purchase - Daily");

                    To_TS = Get_Current_Date_Time(System_Date_Format) + " 23:59:59";
                    From_TS = Add_Secs_Time_String(To_TS, (-86400 * (60 + 1)));

                    Statement = "select ROUND(SUM(Total_Val)), SUBSTR(Create_TS,0,11) Date from Transactions where Payment = 'CREDIT' and In_Out = 'PURCHASE' "
                            + " and Create_TS >= '" + From_TS + "' and Create_TS <= '" + To_TS + "'  group by 2 order by 2 Desc;";

                    break;

                case MCS_Monthly:
                    Purchase_Type_View_set_Text("\u20B9 " + "Purchases - Monthly");
                    ...
                    Statement = "select ROUND(SUM(Total_Val)), SUBSTR(Create_TS,0,8) Date from Transactions where Payment = 'CREDIT' and  In_Out = 'PURCHASE' "
                            + " and Create_TS >= '" + From_TS + "' and Create_TS <= '" + To_TS + "'  group by 2 order by 2 Desc;";

                    break;

                case MCS_Yearly:
                    Purchase_Type_View_set_Text("\u20B9 " + "Purchases - Yearly");
                    ...
                    Statement = "select ROUND(SUM(Total_Val)), SUBSTR(Create_TS,0,5) Date from Transactions where Payment = 'CREDIT' and  In_Out = 'PURCHASE' "
                            + " and Create_TS >= '" + From_TS + "' and Create_TS <= '" + To_TS + "'  group by 2 order by 2 Desc;";
            ...
            if (myDB_G.RunQueryOnce(Statement, Result)) {
                Purchase_Chart_Type_Changed = false;
                DB_Purchase_Chart_Data_Changed = false;
