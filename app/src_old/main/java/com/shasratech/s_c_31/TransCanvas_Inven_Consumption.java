...
public class TransCanvas_Inven_Consumption extends View {
    ...
    public TransCanvas_Inven_Consumption(Context context) {
        super(context);
    }

    public TransCanvas_Inven_Consumption(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public TransCanvas_Inven_Consumption(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public TransCanvas_Inven_Consumption(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void onDraw(Canvas cv) {
        ...
        if (Inven_Consumption_Chart_Type_Changed || DB_Inven_Consumption_Chart_Data_Changed) {
            ...
            Statement = "select ROUND(SUM(Rate * Remain_Quantity)), Consumer from Inventory where In_Out = 'OUT' "
                    + " and Create_Time >= '" + From_TS + "' and Create_Time <= '" + To_TS + "'  group by 2 order by 2;";

            if (myDB_G.RunQueryOnce(Statement, Result)) {
                Inven_Consumption_Chart_Type_Changed = false;
                DB_Inven_Consumption_Chart_Data_Changed = false;
