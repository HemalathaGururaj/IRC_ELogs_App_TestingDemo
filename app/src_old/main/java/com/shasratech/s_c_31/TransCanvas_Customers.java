...
public class TransCanvas_Customers extends View {
    ...
    public TransCanvas_Customers(Context context) {
        super(context);
    }

    public TransCanvas_Customers(Context context, AttributeSet attrs) {
        super(context, attrs);
    }

    public TransCanvas_Customers(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public TransCanvas_Customers(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    protected void onDraw(Canvas cv) {
        ...
        if (Customer_Chart_Type_Changed || DB_Customer_Chart_Data_Changed) {
            switch (main_chart_selection) {
                case MCS_Daily:
                    ...
                    //Statement = "select ROUND(SUM(Total_Val)), Cust_Suplr   from Transactions where In_Out = 'SALE' "
            ...
            Statement = "select ROUND(SUM(Met_Weight)), Cust_Suplr   from Transactions where In_Out = 'SALE' "
                    + " and Create_TS >= '" + From_TS + "' and Create_TS <= '" + To_TS + "'  group by 2 order by 2 ;";

            if (myDB_G.RunQueryOnce(Statement, Result)) {
                Customer_Chart_Type_Changed = false;
                DB_Customer_Chart_Data_Changed = false;
